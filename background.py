...
class Background(pygame.sprite.Sprite):
    def __init__(self, imageFile, levelFile, screenSize):
        ...
        self.tileMap = np.zeros((
            len(mapLine[0].split()),
            len(mapLine))).astype('U256')
        y = 0
        for line in mapLine:
            line = line.split()
            x = 0
    ...
    def move(self, character, vector):
        newPosition = (
            character.position[0] + vector[0],
            character.position[1] + vector[1])
    ...
    def calculateMapSize(self, screenSize):
        ...
        self.player.rect.left = self.player.position[0] * self.tileSize
        self.player.rect.top = self.player.position[1] * self.tileSize

        for character in self.characterArray:
            character.image = pygame.image.load(character.imageFile)
            character.image = pygame.transform.scale(
                character.image,
                (self.tileSize, self.tileSize))
